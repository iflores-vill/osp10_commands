...
output_file=tenant_in-use_quota_$(date +%b-%d-%Y_%H%M).csv

echo "Tenant Name, Tenant ID, in-use Instances, quota Instances, in-use Gigabytes, quota Gigabytes, \
in-use Volumes, quota Volumes, in-use FIPs, quota FIPs, in-use Networks, quota Networks" > $output_file
...
in_use_fips=$(neutron floatingip-list -c id -c fixed_ip_address -c floating_ip_address -c tenant_id | grep -i $tenant_id | wc -l); \
quota_fips=$(openstack quota show $tenant_id | grep -w floating-ips | awk -F "|" '{ printf $3 }'| awk '{ printf $1 }'); \
in_use_networks=$(neutron net-list -c id -c name -c tenant_id | grep $tenant_id | wc -l); \
quota_networks=$(openstack quota show $tenant_id | grep -w networks | awk -F "|" '{ printf $3 }'| awk '{ printf $1 }'); \
echo "$(openstack project show $tenant_id | grep -i name | awk -F '|' '{ printf $3 }'|awk '{ printf $1 }'), \
$tenant_id, $in_use_instances, $quota_instances, $in_use_gigabytes, $quota_gigabytes, $in_use_volumes, $quota_volumes, $in_use_fips, \
$quota_fips, $in_use_networks, $quota_networks" >> $output_file; done
